...
| WildSAT    | wildsat        | Wildlife-guided satellite encoder (WildSAT checkpoint) | S2 → RGB composite   | S2 SR raw → unit/minmax normalization; requires local WildSAT checkpoint (`RS_EMBED_WILDSAT_CKPT`)                             | pooled: image-head/backbone vector [D] ; grid: ViT patch tokens [D, H, W] or [D,1,1] | From WildSAT     |
